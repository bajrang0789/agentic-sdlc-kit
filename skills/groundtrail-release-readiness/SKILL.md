...
license: Apache-2.0
compatibility: Requires current assessment material, release policy inputs, and a writable local workspace; external mutation is allowed only for an explicitly listed action after its Control Point is satisfied.
metadata:
  groundtrail-contract: '1'
  groundtrail-id: 'release-readiness'
---
